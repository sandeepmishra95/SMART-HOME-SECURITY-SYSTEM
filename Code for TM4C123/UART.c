...
void UART_Init(void){
  ...
  GPIO_PORTC_AMSEL_R &= ~0x30;          // disable analog functionality on PC5-4
	...
	//Code for Interrupts
	UART1_IFLS_R = (UART1_IFLS_R & ~0x38) + (0x01<<5); //Configure the FIFO RX interrupt levels
	UART1_IM_R |= 0x40 ; //Set Bits in Interrupt masks for RTIM(Rx Timeout) and RXIM (RX interrupt)
	NVIC_EN0_R |= 0x50 ; //Enable UART1 interrupt 
	NVIC_PRI11_R = (NVIC_PRI11_R & ~0xE00000) + (0x01<<21);
}
...
/* Send a String (Character Array) Using UART*/
...
/*Receive a Message using UART*/
void received_from_UART(char* Buf)
{
	unsigned volatile long i=0;
	do{
		Buf[i]=UART_InCharNonBlocking();
		i++;
	}while(Buf[i-1]!=0);
}
